...
`
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {}
        const material = this.props.material as Record<string, unknown> | undefined
        this.uniforms = material?.uniforms as Uniforms | undefined
      }
      init(..._args: unknown[]) {
      ...
      update(..._args: unknown[]) {
        ...
        Common.renderer.setRenderTarget(
          (this.props.output as THREE.WebGLRenderTarget | null) || null
        )
    ...
    class Advection extends ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        if (!uniforms) return
        if (typeof dt === 'number' && uniforms.dt) uniforms.dt.value = dt
        if (typeof isBounce === 'boolean') this.line.visible = isBounce
        if (typeof BFECC === 'boolean' && uniforms.isBFECC) uniforms.isBFECC.value = BFECC
        super.update()
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { vel } = (args[0] || {}) as { vel?: THREE.WebGLRenderTarget }
